...
bool Spell::playerRuneSpellCheck(Player* player, const Position& toPos, uint16_t runeId)
{
	...
	ReturnValue ret = Combat::canDoCombat(player, tile, aggressive, runeId);
	if (ret != RETURNVALUE_NOERROR) {
		player->sendCancelMessage(ret);
		g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
		return false;
...
{
	if (!playerRuneSpellCheck(player, toPosition, item->getID())) {
		return false;
	}
